...
export function initAttachmentFileUpload() {
...
document.addEventListener("DOMContentLoaded", initAttachmentFileUpload);

// attachment-dropdowns.js
// Sirf single dropdowns ka logic – bilkul tumhare sample jaisa, validation zero

export function initAttachmentDropdowns() {
  const dropdowns = document.querySelectorAll(".attachmentDropdown");

  dropdowns.forEach((dropdown) => {
    const input = dropdown.querySelector(".attachmentDropdown-input");
    const list = dropdown.querySelector(".attachmentDropdown-list");
    const arrow = dropdown.querySelector(".attachmentDropdown-arrow");
    const items = dropdown.querySelectorAll(".attachmentDropdown-item");
    ...
    const toggleDropdown = (shouldOpen) => {
      ...
      document
        .querySelectorAll(".attachmentDropdown-list")
        .forEach((otherList) => {
          if (otherList !== list) {
            otherList.style.maxHeight = "0px";
            otherList.style.opacity = "0";
            const otherArrow = otherList
              .closest(".attachmentDropdown")
              ?.querySelector(".attachmentDropdown-arrow");
  ...
  document.addEventListener("click", (e) => {
    if (!e.target.closest(".attachmentDropdown")) {
      document.querySelectorAll(".attachmentDropdown-list").forEach((list) => {
        list.style.maxHeight = "0px";
        list.style.opacity = "0";
        const arrow = list
          .closest(".attachmentDropdown")
...
// Page load pe start
document.addEventListener("DOMContentLoaded", initAttachmentDropdowns);

// attachment-multi-dropdown.js
export function initAttachmentMultiDropdowns() {
  ...
  dropdowns.forEach((dropdown) => {
    ...
    const toggleDropdown = (shouldOpen) => {
      // Pehle sab dropdowns band kar do (sirf ek khula rahe)
      document
    ...
    // Input pe click → toggle dropdown
    input.addEventListener("click", (e) => {
      e.stopPropagation();
      const isOpen = list.style.maxHeight && list.style.maxHeight !== "0px";
      toggleDropdown(!isOpen);
...
// attachmentFormValidation.js
export function initAttachmentFormValidation() {
  const $form = $("#attachmentForm");
  if (!$form.length) return;

  // ─── Helper Functions (bilkul same as sample) ───────────────────────────────
  ...
  // ─── Required Fields (sirf * wale fields) ───────────────────────────────
  const required = [
    // File Details
    { sel: '[name="fileTitle"]', msg: "File title is required" },
    { sel: '[name="fileType"]', msg: "File type is required" },
    {
      sel: '[name="associatedProject"]',
      msg: "Associated project is required",
    },
    { sel: '[name="associatedTask"]', msg: "Associated task is required" },
    { sel: '[name="visibility"]', msg: "Visibility is required" },

    // Additional Information
    { sel: '[name="category"]', msg: "Category is required" },
  ];
  ...
  $form.on("submit", function (e) {
    ...
    required.forEach((f) => {
      ...
      // Multi-select chip wala field (sirf category multi hai)
      if (f.sel === '[name="category"]') {
    ...
    // ─── Form valid hai → loader + success simulation (tumhare sample jaisa) ───
    if (typeof showGlobalLoader === "function") showGlobalLoader();

    setTimeout(() => {
      if (typeof hideGlobalLoader === "function") hideGlobalLoader();

      // Reset form (optional – agar chahte ho to rakho, warna hata do)
      $form[0].reset();
      $(".selected-items").empty(); // chips clear
  ...
  // ─── Single Dropdown Clear (fileType, associatedProject, associatedTask, visibility) ───
  $form.find(".attachmentDropdown-item").on("click", function () {
    setTimeout(() => {
      const $input = $(this)
        .closest(".attachmentDropdown")
        .find("input")
  ...
  // ─── Multi Dropdown Chips Clear (sirf category) ────────────────────────────
  ...
  // ─── Chip remove hone par check (category ke liye) ────────────────────────────
  $(document).on("click", ".remove-chip", function () {
    setTimeout(() => {
      ...
      if ($container.children().length === 0) {
        ...
        if (name === "category") msg = "Category is required";
